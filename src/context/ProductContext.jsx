...
export const ProductProvider = ({ children }) => {
  ...
  const fetchProductData = useCallback(async (productIdOrSlug) => {
    ...
    const apiUrl = `http://localhost:3001/api/products/${productIdOrSlug}`;
    ...
    try {
      ...
      if (data.product?._id) {
        const imageApiUrl = `http://localhost:3001/api/parts/${data.product._id}/images`;
  ...
  const fetchCommentsForProduct = useCallback(async (entityId, page = 1) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:3001/api/products/${entityId}?commentsPage=${page}&commentsLimit=5`);
      if (!response.ok) {
  ...
  const reportProduct = (productId, reason) => genericReport(`http://localhost:3001/api/products/${productId}/report`, productId, reason);
  const reportComment = (commentId, reason) => genericReport(`http://localhost:3001/api/comments/${commentId}/report`, commentId, reason);
  const reportListing = (listingId, reason) => genericReport(`http://localhost:3001/api/listings/${listingId}/report`, listingId, reason);
  
  const reportImage = useCallback(async (imageId, reason) => {
    if (!token) throw new Error("Authentication required to report images.");
    const response = await fetch(`http://localhost:3001/api/images/${imageId}/report`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
        body: JSON.stringify({ reason }),
    });
  ...
  const voteComment = useCallback(async (commentId) => {
    ...
    const response = await fetch(`http://localhost:3001/api/comments/${commentId}/vote`, {
        method: 'PUT',
  ...
  const downvoteComment = useCallback(async (commentId) => {
    if (!token) throw new Error("Authentication required to vote.");
    const response = await fetch(`http://localhost:3001/api/comments/${commentId}/downvote`, {
        method: 'PUT',
        headers: { 'Authorization': `Bearer ${token}` }
    });
    if (!response.ok) {
  ...
  const deleteUserComment = useCallback(async (commentId) => {
    ...
    await fetch(`http://localhost:3001/api/comments/${commentId}/by-user`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
    });
    setComments(prev => ({
